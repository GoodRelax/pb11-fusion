...
全6レイヤー + D-Tルート、計24の要問。L0-3（燃料選択）で分岐し、p-B11ルート（L1〜L5: 15問）とD-Tルート（5問）が並列する。L0が「そもそもp-B11が最適か？」を問う戦略層。
...
| | p-B11ルート | D-Tルート |
|---|---|---|
| 最大ボトルネック | **物理**（制動放射 P_fus/P_brem） | **工学**（材料・トリチウム） |
| ボトルネックの性質 | 原理的に超えられるか不明 | 時間と金で解ける可能性が高い |
...
```
■ p-B11ルート（L0〜L5: 19問）
🟢 解決済み   : L2-1（タイミング同期）, L4-3（燃料供給）                    → 2/19
🟡 部分解決   : L0-1, L0-3, L0-4, L1-3, L2-3, L3-3, L4-1                   → 7/19
🔴 未解決     : L0-2, L1-1, L1-2, L2-2, L3-1, L3-2, L4-2, L5               → 8/19
...
| | p-B11ルート | D-Tルート |
|---|---|---|
| 最大ボトルネック | **L1-2 制動放射**（物理の壁） | **DT-3 照射試験施設**（工学の谷） |
| ボトルネックの性質 | 原理的に超えられるか不明 | 時間と金で解ける可能性が高い |
| 解決の見通し | Ti/Te ≈ 300 必要。4サイクルで60〜150？ | DONES完成 2030年代半ば → データ蓄積 2040年代 |
| 失敗した場合 | p-B11は発電燃料として不可能 | D-T商用炉の時期が遅れる |

**p-B11ルート最大のボトルネック：** L1-2（制動放射問題）
...
_最終更新: 2026-04-05 — D-Tルート（5問）をL0分岐として追加。合計24問体制_
